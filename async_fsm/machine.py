...
class Machine:
    def __init__(self, state_factory, init_state, other_states, transitions):
        self._current_state = state_factory.create(init_state)
        self._states = {s: state_factory.create(s) for s in other_states}
        self._transitions = [Transition(*t) for t in transitions]
...
async def create_machine(state_factory, init_state, other_states, transitions):
    m = Machine(state_factory, init_state, other_states, transitions)
